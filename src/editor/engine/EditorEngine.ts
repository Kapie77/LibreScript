...
// =====================================================
    ...
    private onDocumentChangedListeners: Array<(blocks: ScriptBlock[]) => void> = [];
    private onHistoryChangedListeners: Array<(history: ExecutedCommand[]) => void> = [];
    ...
    const history = this.history.getUndoHistory();

    for (const listener of this.onHistoryChangedListeners) {
        listener(history);
    }
}
...
// =====================================================
    ...
    public setZoom(zoom: number): void {
        ...
        this.documentView.setZoom(zoom);
    }
    ...
    public setParagraphAlignment(
        ...
        this.execute({
            ...
            paragraphId: selection.anchorParagraphId,
            alignment,
            selection,
        });
    ...
    // getHistory //
    public getHistory(): ExecutedCommand[] {
        return this.history.getUndoHistory();
    }

    // onHistoryChanged //
    public onHistoryChanged(
        listener: (history: ExecutedCommand[]) => void
    ) {

        this.onHistoryChangedListeners.push(listener);

        return () => {

            const index =
                this.onHistoryChangedListeners.indexOf(
                    listener
                );

            if (index !== -1) {

                this.onHistoryChangedListeners.splice(
                    index,
                    1
                );
        ...
        try {
            ...
            switch (executed.command.type) {
                ...
                case "CHANGE_BLOCK_TYPE": {
                    ...
                    const undo =
                        executed.undoData as ParagraphTypeUndoData;
