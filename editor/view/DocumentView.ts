...
export class DocumentView {
    ...
    private editorScrollContainer: HTMLDivElement | null = null;
    ...
    constructor(
        ...
        this.onSave = onSave;
        this.editorScrollContainer = this.root.closest<HTMLDivElement>(".document-editor");
        ...
        this.root.addEventListener(
            "pointerdown",
            this.handleRootPointerDown
        );

        this.editorScrollContainer?.addEventListener(
            "scroll",
    ...
    private handleRootPointerDown = (event: PointerEvent): void => {
        ...
        const page = target.closest<HTMLElement>(".document-editor-page");
        ...
        const fragment = target.closest<HTMLParagraphElement>(
            "p[data-paragraph-id]"
        );

        if (!fragment) return;

        const paragraphId = Number(fragment.dataset.paragraphId);

        if (!Number.isFinite(paragraphId)) return;

        const localOffset = this.selection.getClosestOffsetInFragment(
            fragment,
            event.clientX,
            event.clientY
        );
        ...
        const logicalOffset = fragmentStart + localOffset;
    ...
    private handleDocumentPointerDown = (
        ...
        const insideEditor =
            target.closest(".document-editor-page") !== null;
    ...
    // updateVisualCaret //
    private updateVisualCaret(
        restartBlink: boolean = true
    ): void {
        ...
        if (!this.caretElement) {
            ...
            this.caretElement.className =
                "document-editor-custom-caret";
    ...
    // updateVisualCaretFromLogicalPosition //
    private updateVisualCaretFromLogicalPosition(
        paragraphId: number,
        offset: number,
        restartBlink: boolean = true
    ): void {
        ...
        const point =
            this.selection.getDomPointFromOffset(
                paragraphId,
                offset
            );
    ...
    private restartCaretBlink(): void {
        ...
        // Força o browser a reiniciar a animação.
        void this.caretElement.offsetWidth;

        this.caretElement.style.animation =
            "document-editor-caret-blink 1s step-end infinite";
    }

    // hideVisualCaret //
    private hideVisualCaret(): void {
    ...
    // refreshCaretAfterZoom //
    private refreshCaretAfterZoom(
        paragraphId: number,
        offset: number
    ): void {
    ...
    render(paragraphs: Paragraph[]) {
            ...
            while (
                ...
                // Se não couber, vamos diminuindo.
                    ...
                    while (
                        ...
                        fragment.style.textAlign = layout.align;
        ...
        // -----------------------------------------------------
        // APLICA ALINHAMENTOS PERSONALIZADOS
        // -----------------------------------------------------
        //
        // A paginação já terminou.
        //
        // Até aqui todos os fragmentos foram medidos usando
        // layout.align, preservando a geometria original dos
        // blocos de screenplay.
        //
        // Agora podemos aplicar o alinhamento escolhido pelo
        // usuário somente dentro das caixas já posicionadas.
        // -----------------------------------------------------
    ...
    destroy() {
        ...
        this.editorScrollContainer?.removeEventListener(
            "scroll",
            this.handleEditorScroll
        );
    ...
    private applyParagraphAlignment(
        ...
        const fragments =
            this.root.querySelectorAll<HTMLParagraphElement>(
