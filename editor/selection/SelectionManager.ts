...
export class SelectionManager {
    ...
    */
    ...
    // extendSelection //
    extendSelection(paragraphId: number, offset: number) {
        const anchor = this.controller.getAnchor();

        console.log("EXTEND ANTES", {
            anchor,
            paragraphId,
            offset,
        });
        ...
        console.log("EXTEND DEPOIS", {
            anchor: this.controller.getAnchor(),
            focus: this.controller.getFocus(),
        });

        this.renderSelection();

        console.log("EXTEND FINAL", {
            anchor: this.controller.getAnchor(),
            focus: this.controller.getFocus(),
        });
    }
    ...
    // getLogicalParagraphText //
    private getLogicalParagraphText(
        paragraphId: number
    ): string {
        ...
        const ordered =
            [...fragments].sort(
                (a, b) =>
                    Number(a.dataset.startOffset ?? 0) -
                    Number(b.dataset.startOffset ?? 0)
            );
        ...
        const selectedParagraphs =
            paragraphIds
                ...
                .map(id => {
                    ...
                    const fragment =
                        this.getFirstVisualFragment(id);
                    ...
                    return {
                        ...
                        content:
                            this.getLogicalParagraphText(id),
    ...
    // ==============================
    // getDomPointFromoffset
    // ==============================
    public getDomPointFromOffset(
        paragraphId: number,
        offset: number
    ): {
        node: Text;
